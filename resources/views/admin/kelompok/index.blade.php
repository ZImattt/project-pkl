...
@section('title', 'Data Pendaftaran Kelompok - Admin Global Intermedia')
...
<div class="dashboard-header">
    <div>
        <h1 class="page-title">Data Pendaftaran Kelompok</h1>
        <p class="page-subtitle">Kelola pendaftaran kelompok PKL & Magang</p>
    </div>
    <div>
        <span class="badge-total"><i class="fas fa-users"></i> Total: <strong>{{ $kelompoks->total() }} kelompok</strong></span>
    </div>
</div>
...
<div class="filter-section">
    <div class="filter-row">
        ...
        <div class="filter-pills">
            <a href="{{ route('admin.kelompok.index', array_merge(request()->except('status'), ['status' => 'all'])) }}" class="filter-pill {{ request('status', 'all') == 'all' ? 'active' : '' }}">Semua</a>
            <a href="{{ route('admin.kelompok.index', array_merge(request()->except('status'), ['status' => 'pending'])) }}" class="filter-pill {{ request('status') == 'pending' ? 'active' : '' }}">Pending</a>
            <a href="{{ route('admin.kelompok.index', array_merge(request()->except('status'), ['status' => 'diterima'])) }}" class="filter-pill {{ request('status') == 'diterima' ? 'active' : '' }}">Diterima</a>
            <a href="{{ route('admin.kelompok.index', array_merge(request()->except('status'), ['status' => 'ditolak'])) }}" class="filter-pill {{ request('status') == 'ditolak' ? 'active' : '' }}">Tidak Diterima</a>
        </div>
    </div>
    <div class="filter-row">
        <span class="filter-label-text">Pendidikan</span>
        <div class="filter-pills">
            <a href="{{ route('admin.kelompok.index', array_merge(request()->except('jenis_pendidikan'), ['jenis_pendidikan' => 'all'])) }}" class="filter-pill {{ request('jenis_pendidikan', 'all') == 'all' ? 'active' : '' }}">Semua</a>
            <a href="{{ route('admin.kelompok.index', array_merge(request()->except('jenis_pendidikan'), ['jenis_pendidikan' => 'smk'])) }}" class="filter-pill {{ request('jenis_pendidikan') == 'smk' ? 'active' : '' }}">SMK</a>
            <a href="{{ route('admin.kelompok.index', array_merge(request()->except('jenis_pendidikan'), ['jenis_pendidikan' => 'kuliah'])) }}" class="filter-pill {{ request('jenis_pendidikan') == 'kuliah' ? 'active' : '' }}">Perguruan Tinggi</a>
        </div>
    </div>
    <div class="filter-row">
        <div class="search-box">
            <i class="fas fa-search"></i>
            <input type="text" id="searchInput" placeholder="Cari nama kelompok, kode, atau institusi..." value="{{ request('search') }}">
        </div>
    </div>
</div>
...
<div class="data-container">
    @if($kelompoks->count() > 0)
        <!-- DESKTOP TABLE - KOLOM DIPERKECIL AGAR BUTTON 3 BERJEJER -->
                ...
                <table class="table">
                    <thead>
                        <tr>
                            <th width="25">No</th>
                            <th width="70">Kode</th>
                            <th width="120">Nama Kelompok</th>
                            <th width="95">Institusi</th>
                            <th width="90">Perwakilan</th>
                    ...
                    <tbody>
                        @foreach($kelompoks as $index => $kelompok)
                            @php
                                $anggotaCount = $kelompok->anggota_count ?? $kelompok->anggota->count();
                                $waNumber = preg_replace('/[^0-9]/', '', $kelompok->perwakilan_wa);
                                $statusClass = $kelompok->status;
                                $statusText = match($kelompok->status) {
                                    'pending' => 'Pending',
                                    'diterima' => 'Diterima',
                                    'ditolak' => 'Tidak Diterima',
                                    default => $kelompok->status
                                };
                                $itemNumber = ($kelompoks->currentPage() - 1) * $kelompoks->perPage() + $loop->iteration;
                            @endphp
                            <tr id="row-{{ $kelompok->id }}">
                                <td class="text-center">{{ $itemNumber }}</td>
                                <td class="text-center">
                                    <span class="kode-badge" onclick="copyKode('{{ $kelompok->kode_pendaftaran }}', this)">{{ $kelompok->kode_pendaftaran }}</span>
                                </td>
                                <td>
                                    <div class="text-truncate"><strong>{{ $kelompok->nama_kelompok }}</strong></div>
                                </td>
                                <td class="text-center">
                                    <div class="text-truncate">{{ $kelompok->institusi }}</div>
                                    <small class="text-muted">{{ $kelompok->perwakilan_jenis_pendidikan == 'smk' ? 'SMK' : 'PT' }}</small>
                                </td>
                                <td class="text-center">
                                    <div class="text-truncate">{{ $kelompok->perwakilan_nama }}</div>
                                </td>
                                <td class="text-center">
                                    <a href="https://wa.me/{{ $waNumber }}" target="_blank" class="wa-link"><i class="fab fa-whatsapp"></i> {{ substr($kelompok->perwakilan_wa, 0, 12) }}</a>
                                </td>
                                <td class="text-center">
                                    <span class="badge-anggota">{{ $anggotaCount }}/{{ $kelompok->jumlah_anggota }}</span>
                                </td>
                                <td class="text-center">{{ $kelompok->created_at->format('d/m/Y') }}</td>
                                <td class="text-center">
                                    <span class="badge-status {{ $statusClass }}" id="status-{{ $kelompok->id }}">{{ $statusText }}</span>
                                </td>
                                <td class="text-center">
                                    <div class="action-group">
                                        <button onclick="openUpdateModal({{ $kelompok->id }}, '{{ $kelompok->status }}', '{{ addslashes($kelompok->catatan_admin ?? '') }}', '{{ addslashes($kelompok->nama_kelompok) }}', '{{ $kelompok->kode_pendaftaran }}')" class="btn-action btn-edit" title="Update"><i class="fas fa-edit"></i></button>
                                        <a href="{{ route('admin.kelompok.show', $kelompok->id) }}" class="btn-action btn-detail" title="Detail"><i class="fas fa-eye"></i></a>
                                        <button onclick="confirmDelete({{ $kelompok->id }}, '{{ addslashes($kelompok->nama_kelompok) }}')" class="btn-action btn-delete" title="Hapus"><i class="fas fa-trash"></i></button>
                                    </div>
                                </td>
                            </tr>
                        @endforeach
                ...
                <div class="mobile-card">
                    <div class="mobile-card-header" onclick="toggleMobileCard(this)">
                        <div style="flex:1;min-width:0;">
                            <div class="mobile-name">{{ $kelompok->nama_kelompok }}</div>
                            <div class="mobile-meta">
                                <span>{{ $kelompok->perwakilan_nama }}</span>
                                <span>•</span>
                                <span>{{ $kelompok->institusi }}</span>
                            </div>
                        </div>
                        <div style="display:flex;align-items:center;gap:8px;flex-shrink:0;">
                            <span class="badge-status {{ $kelompok->status }}" style="font-size:0.55rem;padding:2px 8px;">{{ $statusText }}</span>
                    ...
                    <div class="mobile-card-body">
                        <div class="mobile-row">
                            ...
                            <div class="mobile-value"><span class="kode-badge" onclick="event.stopPropagation();copyKode('{{ $kelompok->kode_pendaftaran }}',this)" style="font-size:0.6rem;">{{ $kelompok->kode_pendaftaran }}</span></div>
                        </div>
                        <div class="mobile-row"><div class="mobile-label">Institusi</div><div class="mobile-value">{{ $kelompok->institusi }} ({{ $kelompok->perwakilan_jenis_pendidikan == 'smk' ? 'SMK' : 'PT' }})</div></div>
                        <div class="mobile-row"><div class="mobile-label">Perwakilan</div><div class="mobile-value">{{ $kelompok->perwakilan_nama }}</div></div>
                        <div class="mobile-row"><div class="mobile-label">WhatsApp</div><div class="mobile-value"><a href="https://wa.me/{{ $waNumber }}" target="_blank" class="wa-link">{{ $kelompok->perwakilan_wa }}</a></div></div>
                        <div class="mobile-row"><div class="mobile-label">Anggota</div><div class="mobile-value">{{ $anggotaCount }}/{{ $kelompok->jumlah_anggota }}</div></div>
                        <div class="mobile-row"><div class="mobile-label">Tanggal</div><div class="mobile-value">{{ $kelompok->created_at->format('d M Y') }}</div></div>
                        @if($kelompok->catatan_admin)
                        <div class="mobile-row"><div class="mobile-label">Catatan</div><div class="mobile-value">{{ $kelompok->catatan_admin }}</div></div>
                        @endif
                        <div class="mobile-actions">
                            <button onclick="event.stopPropagation();openUpdateModal({{ $kelompok->id }},'{{ $kelompok->status }}','{{ addslashes($kelompok->catatan_admin ?? '') }}','{{ addslashes($kelompok->nama_kelompok) }}','{{ $kelompok->kode_pendaftaran }}')" class="mobile-btn mobile-btn-edit"><i class="fas fa-edit"></i> Update</button>
                            <a href="{{ route('admin.kelompok.show', $kelompok->id) }}" class="mobile-btn mobile-btn-detail" onclick="event.stopPropagation();"><i class="fas fa-eye"></i> Detail</a>
                            ...
                            <button onclick="event.stopPropagation();confirmDelete({{ $kelompok->id }},'{{ addslashes($kelompok->nama_kelompok) }}')" class="mobile-btn mobile-btn-delete"><i class="fas fa-trash"></i> Hapus</button>
                        </div>
                    </div>
                </div>
            @endforeach
        ...
        <div class="pagination-wrapper">
            <ul class="pagination">
                @if ($kelompoks->onFirstPage())
                    <li class="page-item disabled"><span class="page-link">&laquo; Prev</span></li>
                @else
                    <li class="page-item"><a class="page-link" href="{{ $kelompoks->previousPageUrl() }}" rel="prev">&laquo; Prev</a></li>
                @endif

                @foreach ($kelompoks->getUrlRange(1, $kelompoks->lastPage()) as $page => $url)
                    @if ($page == $kelompoks->currentPage())
                ...
                @if ($kelompoks->hasMorePages())
                    <li class="page-item"><a class="page-link" href="{{ $kelompoks->nextPageUrl() }}" rel="next">Next &raquo;</a></li>
                @else
    ...
    @else
        <div class="empty-state">
            ...
            <p class="text-muted">Belum ada pendaftaran kelompok</p>
        </div>
    @endif
</div>
...
<div class="update-modal-overlay" id="updateModalOverlay">
    <div class="update-modal-container">
        <div class="update-modal-header">
            <h3><i class="fas fa-edit" style="color:var(--red);margin-right:6px;"></i> Update Status Kelompok</h3>
            <button class="update-modal-close" onclick="closeUpdateModal()"><i class="fas fa-times"></i></button>
        </div>
        <div class="update-modal-body">
            <input type="hidden" id="updateId">
            <div class="modal-info-card">
                <div class="modal-info-item"><strong>Kelompok:</strong><span id="infoNama">-</span></div>
    ...
    <div class="modal-dialog modal-dialog-centered modal-sm">
        <div class="modal-content">
            ...
            <div class="modal-body text-center">
                ...
                <p class="mb-1">Yakin ingin menghapus kelompok?</p>
                <p class="text-muted small"><strong id="deleteItemName"></strong></p>
                <p class="text-danger small">Semua anggota juga akan terhapus!</p>
            </div>
...
function submitUpdate() {
    ...
    $.ajax({
        url: `/admin/kelompok/${id}/status`, method: 'PUT',
...
function confirmDelete(id, name) { currentDeleteId = id; document.getElementById('deleteItemName').textContent = name; deleteModal.show(); }
...
function executeDelete() {
    ...
    $.ajax({
        url: `/admin/kelompok/${currentDeleteId}`, method: 'DELETE', data: { _token: '{{ csrf_token() }}' },
        success: function(r) {
            document.getElementById('loadingOverlay').style.display = 'none';
            if (r.success) {
